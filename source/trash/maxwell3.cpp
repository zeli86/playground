...
class ZCurrent :  public AutoDerivativeFunction<3>
{
public:
  ZCurrent ();
...
ZCurrent::ZCurrent () : AutoDerivativeFunction<3> (0.001, 3)
{
  A=0.01;
  R0=1;
  Z0=0;
...
inline double ZCurrent::value (const Point<3> &p, const unsigned int component ) const
{
  ...
  switch(component)
  {
    case 0: retval=fabs(tmp1)*exp(-fabs(y-tmp1))*exp(-z*z); break;
    case 1: retval=(1-tmp1*tmp1)*exp(-fabs(y-tmp1))*exp(-z*z); break;
  }
  
return retval;
}

inline void ZCurrent::vector_value (const Point<3> &p, Vector<double> &values)  const 
{
  double x = p(0);
  double y = p(1);
  double z = p(2);
  ...
  double tmp1=tanh(x);
  values(0) = fabs(tmp1)*exp(-fabs(y-tmp1))*exp(-z*z);
  values(1) = (1-tmp1*tmp1)*exp(-fabs(y-tmp1))*exp(-z*z);
  values(2) = 0;
}
