...
namespace HelperPrograms
{
  ...
  template<int dim>
  double MySolver<dim>::compute_inf_norm()
  {
    double retval=0, tmp1=0, JxW;
    ...
    MPI_Allreduce( &tmp1, &retval, 1, MPI_DOUBLE, MPI_MAX, mpi_communicator);
    return retval;
  }  
  ...
  {
    string filename2 = dirname(filename);
    if( filename2 == "" )
      filename2 = "info.xml";
    else
      filename2 += "/info.xml";
    ...
    load(filename);
    pcout << m_mu << "\t" << compute_inf_norm() << endl;
  }
