...
namespace BreedSolver
{
  ...
  {
    make_grid();
    triangulation.load( "out-1.bin" );

    setup_system();

    parallel::distributed::SolutionTransfer<dim,LA::MPI::Vector> solution_transfer(dof_handler);
    solution_transfer.deserialize(m_vec_ng);
    constraints.distribute(m_vec_ng);
    m_vec=m_vec_ng;
  }    
  ...
  void MySolver<dim>::output_results ()
  {
    ...
    data_out.write_vtu_in_parallel ( "out-2.vtu", mpi_communicator);
  }
