 ...
 */
      ...
      if( cell->is_locally_owned() )
      {
        ...
        for ( unsigned qp=0; qp<n_q_points; ++qp )
        {
          ...
          for (unsigned int i=0; i<dofs_per_cell; ++i)
            cell_rhs(i) += JxW*(grads[qp][0]*fe_values[rt].gradient(i,qp) + (Pot+m_gs*(req-3*imq))*vals[qp][0]*fe_values[rt].value(i,qp) + m_Gamma*vals[qp][1]*fe_values[rt].value(i,qp) 
    ...
    for ( ; cell!=endc; ++cell )
    {
      if( cell->is_locally_owned() )
      {
        ...
        for ( unsigned qp=0; qp<n_q_points; ++qp )
        {
          req = vals[qp][0]*vals[qp][0];
          imq = vals[qp][1]*vals[qp][1];
          psum += fe_values.JxW(qp)*( grads[qp][0]*grads[qp][0] - grads[qp][1]*grads[qp][1] + (Potential.value(fe_values.quadrature_point(qp))-m_mu)*(req-imq) + 0.5*m_gs*(req*req+imq*imq-6*req*imq) - m_Gamma*vals[qp][0]*vals[qp][1] ); 
        }
      }
    }
    psum *= 0.5;
  ...
  void MySolver<dim>::find_min_J()
  {
    ...
    gsl_vector_set (x, 0, m_ti);
    gsl_vector_set (x, 1, m_ti);
