...
class Coil :  public AutoDerivativeFunction<3>
{
public:
  Coil ( const double );
...
Coil::Coil ( const double a ) : AutoDerivativeFunction<3> (0.001, 3)
{
  alp=10;
  A=0.01;
  R0=1;
...
inline double Coil::value (const Point<3> &p, const unsigned int component ) const
{
  ...
  double tmp1= exp( -0.5*( (r-R0)*(r-R0)/pow(sigma_r,2) + (z-Z0)*(z-Z0)/pow(sigma_r,2) ) );
  double tmp2 = atan2(y,x);
  
  switch(component)
  {
    case 0: retval=-sin(tmp2)*tmp1; break;
    case 1: retval=cos(tmp2)*tmp1; break;
  }
return retval;
}

inline void Coil::vector_value (const Point<3> &p, Vector<double> &values)  const 
{
  double x = p(0);
  double y = p(1);
  double z = p(2);
  ...
  double tmp1= exp( -0.5*( (r-R0)*(r-R0)/pow(sigma_r,2) + (z-Z0)*(z-Z0)/pow(sigma_r,2) ) );
  double tmp2 = atan2(y,x);
  values(0) = -sin(tmp2)*tmp1;
  values(1) = cos(tmp2)*tmp1;
  values(2) = 0;
}
...
void MySolver::setup_system()
{
  ...
  //VectorTools::project_boundary_values_curl_conforming(dof_handler, 0,ConstantFunction<3>(1.0, fe.n_components()), 0, constraints);
  constraints.close ();
...
void MySolver::assemble_0 ()
{
  ...
  Coil coil1(-Zh);
  Coil coil2(Zh);
  vector<Vector<double> > rhs_values_1 (n_q_points,Vector<double>(3));
  vector<Vector<double> > rhs_values_2 (n_q_points,Vector<double>(3));
  ...
  for (; cell!=endc; ++cell)
  {
    if( cell->is_locally_owned() )
    {
      ...
      coil1.vector_value_list (fe_values.get_quadrature_points(), rhs_values_1);
      coil2.vector_value_list (fe_values.get_quadrature_points(), rhs_values_2);
	  ...
	  cell_rhs(i) += (dotprod(value_i,rhs_values_1[q_point]) + dotprod(value_i,rhs_values_2[q_point]) )*fe_values.JxW(q_point);
        }
      }
...
void MySolver::assemble_B ()
{
  ...
  Coil coil1(-Zh);
  Coil coil2(Zh);
  vector<vector<Tensor<1,3>>> rhs_values_grads_1(n_q_points, vector<Tensor<1,3>>(3));  
  vector<vector<Tensor<1,3>>> rhs_values_grads_2(n_q_points, vector<Tensor<1,3>>(3));  
  ...
  for (; cell!=endc; ++cell)
  {
    if( cell->is_locally_owned() )
    {
      ...
      cell_rhs = 0;
      fe_values.reinit (cell);
      coil1.vector_gradient_list (fe_values.get_quadrature_points(), rhs_values_grads_1);
      coil2.vector_gradient_list (fe_values.get_quadrature_points(), rhs_values_grads_2);
      ...
      for (unsigned int q_point=0; q_point<n_q_points; ++q_point)
      {
        for (unsigned int i=0; i<dofs_per_cell; ++i)
        {
          ...
          for (unsigned int j=0; j<dofs_per_cell; ++j)
          {
	    cell_matrix(i,j) += fe_values.JxW(q_point)*(fe_values[A].curl(i,q_point)*fe_values[A].curl(j,q_point)+0.5*fe_values[A].value(i,q_point)*fe_values[A].value(j,q_point));
          }
          rot_rhs[0] = rhs_values_grads_1[q_point][2][1]-rhs_values_grads_1[q_point][1][2];
	  rot_rhs[1] = rhs_values_grads_1[q_point][0][2]-rhs_values_grads_1[q_point][2][0];
	  rot_rhs[2] = rhs_values_grads_1[q_point][1][0]-rhs_values_grads_1[q_point][0][1];
          rot_rhs[0] += (rhs_values_grads_2[q_point][2][1]-rhs_values_grads_2[q_point][1][2]);
	  rot_rhs[1] += (rhs_values_grads_2[q_point][0][2]-rhs_values_grads_2[q_point][2][0]);
	  rot_rhs[2] += (rhs_values_grads_2[q_point][1][0]-rhs_values_grads_2[q_point][0][1]);
