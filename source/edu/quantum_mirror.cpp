...
namespace edu
{
  ...
  /*************************************************************************************************/
  template <int dim>
  class CWavefunction : public Function<dim>
  {
    public:
      CWavefunction () : Function<dim>(2) {  }
  ...
  template <int dim> 
  inline void CWavefunction<dim>::vector_value (const Point<dim> &p, Vector<double> &values) const
  {
    constexpr double shift=2.5;
    constexpr double shiftq=shift*shift;
    constexpr double sigmaq=0.3;
    
    values[0] = exp( -0.5 * (p*p-2*shift*p(dim-1)+shiftq) / sigmaq ) / pow( M_PI*sigmaq, 0.25*double(dim)); 
    values[1] = 0;
  }

  template <int dim>
  void CWavefunction<dim>::vector_value_list (const std::vector<Point<dim>> &points, std::vector<Vector<double>> &value_list) const
  {
    const unsigned n_points = points.size();
    for( unsigned p=0; p<n_points; ++p )
      CWavefunction<dim>::vector_value (points[p], value_list[p]);
  }
  ...
  void MySolver<dim>::run()
  {
    ...
    std::string mesh_filename = "dome_" + std::to_string(dim) + "d.msh";
...
int main ( int argc, char *argv[] )
{
  ...
  {
    edu::MySolver<3> solver("params.xml");
    solver.run();
  }  
return EXIT_SUCCESS;
}