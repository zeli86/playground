...
namespace BreedSolver_1
{
  ...
  template <int dim> class MySolver
  {
  public:
  ...
  template <int dim>
  void MySolver<dim>::compute_E_lin(Vector<double> &vec, double &T, double &N,
                                    double &W)
  {
  ...
  void MySolver<dim>::assemble_rhs()
  {
    ...
    constraints.condense(m_system_rhs);
  }
  ...
  void MySolver<dim>::compute_Psi_sob()
  {
    ...
    for (; cell != endc; cell++)
    {
      ...
      for (unsigned qp = 0; qp < n_q_points; qp++)
      {
        ...
        for (unsigned i = 0; i < dofs_per_cell; i++)
          m_system_rhs(local_dof_indices[i]) += JxW * vals[qp] * fe_values.shape_value(i, qp);
      }
    }
    constraints.condense(m_system_rhs);
    solve();
  ...
  template <int dim> void MySolver<dim>::setup_system()
  {
  ...
  template <int dim> int MySolver<dim>::DoIter(string path)
  {
  ...
  template <int dim> void MySolver<dim>::save(string filename) 
  {
    ofstream ofs(filename);
    m_Psi.block_write(ofs);
  }

  template <int dim> void MySolver<dim>::dump_info_xml(const string path)
  {
    /*
