...
namespace BreedSolver
{
  ...
  protected:
    ...
    double l2norm_t()
    {
      return sqrt(m_t[0] * m_t[0] + m_t[1] * m_t[1]);
    };
    ...
    for (; cell != endc; ++cell)
    {
      if (cell->is_locally_owned())
      {
        ...
        for (unsigned qp = 0; qp < n_q_points; ++qp)
        {
          ...
          psum += fe_values.JxW(qp) * fabs(fe_values.quadrature_point(qp)[1]) * (grads[qp][0] * grads[qp][0] + grads[qp][1] * grads[qp][1] + (Potential.value(fe_values.quadrature_point(qp)) - m_rMu) * uq + 0.5 * m_gs * uq * uq);
        }
      }
    }
    psum *= 0.5;
  ...
  void MySolver<dim>::run2b()
  {
    ...
    for (int i = 0; i < m_Ndmu; ++i)
    {
      ...
      // nehari
      //m_ti = sqrt((m_rMu*N-T)/(4.0*m_gs*W)); // if m_Psi_2 == m_Psi_1
      m_ti = sqrt(2 * (m_rMu * N - T) / (m_gs * W));
