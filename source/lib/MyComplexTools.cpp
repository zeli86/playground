...
namespace MyComplexTools
{
                              ...
                              Vector<double>& rhs)
  {
    ...
    for (; cell != endc; ++cell)
    {
      ...
      for (unsigned qp = 0; qp < n_q_points; ++qp)
      {
        ...
        const double c = vals[qp][0];
        const double d = vals[qp][1];
        const double phi = -dt * (gam * (vals[qp][0] * vals[qp][0] + vals[qp][1] * vals[qp][1]) + Potential.value(fe_values.quadrature_point(qp)));
        double a, b;
        sincos(phi, &b, &a);
  ...
  template <int dim>
  double Particle_Number(const DoFHandler<dim>& dof_handler, const FESystem<dim>& fe, const Vector<double>& vec)
  {
    double retval = 0;
    const QGauss<dim> quadrature_formula(fe.degree + 1);
    const FEValuesExtractors::Scalar rt(0);
  ...
  template double Particle_Number(const DoFHandler<2>&, const FESystem<2>&, const Vector<double>&);
  template double Particle_Number(const DoFHandler<3>&, const FESystem<3>&, const Vector<double>&);
...
namespace MyComplexTools
{
  namespace MPI
  {
    ...
    void AssembleSystem_mulvz(const DoFHandler<dim>& dof_handler,
                              ...
                              const std::complex<double> z,
                              LA::MPI::SparseMatrix& matrix, LA::MPI::Vector& rhs)
    {
      assert(vec.has_ghost_elements() == true);
    ...
    void AssembleSystem_LIN_Step(const DoFHandler<dim>& dof_handler,
                                 const FESystem<dim>& fe,
                                 const dealii::AffineConstraints<double>& constraints,
                                 const LA::MPI::Vector& vec, const double dt,
                                 LA::MPI::SparseMatrix& matrix,
                                 LA::MPI::Vector& rhs)
    {
      assert(vec.has_ghost_elements() == true);
    ...
    double Particle_Number(MPI_Comm mpi_communicator,
                           ...
                           const FESystem<dim>& fe, const LA::MPI::Vector& vec)
    {
      assert(vec.has_ghost_elements() == true);
    ...
    template double Particle_Number<2>(MPI_Comm, const DoFHandler<2>&, const FESystem<2>&, const LA::MPI::Vector&);
    template double Particle_Number<3>(MPI_Comm, const DoFHandler<3>&, const FESystem<3>&, const LA::MPI::Vector&);
