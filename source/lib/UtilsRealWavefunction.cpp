 ...
 */
template<int iDim>
double particle_number(IRealWavefunction<iDim>* pBase, const Vector<double>& vWavefunction)
{
  double retval{ 0 };
  ...
  FEValues<iDim> fe_values(
    fe, quadrature_formula, update_values | update_quadrature_points | update_JxW_values);

  const unsigned n_q_points = quadrature_formula.size();

  std::vector<double> vals(n_q_points);

  typename DoFHandler<iDim>::active_cell_iterator cell = dof_handler.begin_active(),
...
template double particle_number<1>(IBase<DoFHandler<1>, FE_Q<1>, AffineConstraints<double>>*,
                                   const Vector<double>&);
template double particle_number<2>(IBase<DoFHandler<2>, FE_Q<2>, AffineConstraints<double>>*,
                                   const Vector<double>&);
 ...
 */
template<int iDim>
double particle_number(IRealWavefunction<iDim>* pBase,
...
template double particle_number<2>(IBase<DoFHandler<2>, FE_Q<2>, AffineConstraints<double>>*,
                                   const LA::MPI::Vector&,
                                   MPI_Comm);
template double particle_number<3>(IBase<DoFHandler<3>, FE_Q<3>, AffineConstraints<double>>*,
                                   const LA::MPI::Vector&,
                                   MPI_Comm);
 ...
 * @param pBase
 * @param vWavefunction
 * @param oParticleNumber
 * @return std::array<double, iDim>
 */
template<int iDim>
std::array<double, iDim> expectation_value_position(IRealWavefunction<iDim>* pBase,
...
template std::array<double, 1> expectation_value_position<1>(
  IBase<DoFHandler<1>, FE_Q<1>, AffineConstraints<double>>*,
  const Vector<double>&,
  const std::optional<double>);
template std::array<double, 2> expectation_value_position<2>(
  IBase<DoFHandler<2>, FE_Q<2>, AffineConstraints<double>>*,
  const Vector<double>&,
  const std::optional<double>);
...
template std::array<double, 1> expectation_value_position<1>(
  IBase<DoFHandler<1>, FE_Q<1>, AffineConstraints<double>>*, const Vector<double>&);
template std::array<double, 2> expectation_value_position<2>(
  IBase<DoFHandler<2>, FE_Q<2>, AffineConstraints<double>>*, const Vector<double>&);
                                                    ...
                                                    std::optional<double> oParticleNumber)
{
  ...
  MPI_Allreduce(
    oLocalIntegral.begin_raw(), oTotalIntegral.data(), iDim, MPI_DOUBLE, MPI_SUM, mpi_communicator);
...
template std::array<double, 2> expectation_value_position<2>(IRealWavefunction<2>*,
                                                             const LA::MPI::Vector&,
                                                             MPI_Comm,
                                                             const std::optional<double>);
template std::array<double, 3> expectation_value_position<3>(IRealWavefunction<3>*,
                                                             const LA::MPI::Vector&,
                                                             MPI_Comm,
                                                             const std::optional<double>);
...
template<int iDim>
std::array<double, iDim> expectation_value_width(IRealWavefunction<iDim>* pBase,
                                                 const Vector<double>& vWavefunction,
...
template std::array<double, 1> expectation_value_width<1>(IRealWavefunction<1>*,
                                                          const Vector<double>&,
                                                          const Point<1>&,
                                                          const std::optional<double>);
template std::array<double, 2> expectation_value_width<2>(IRealWavefunction<2>*,
                                                          const Vector<double>&,
                                                          const Point<2>&,
                                                          const std::optional<double>);
/**
...
template std::array<double, 2> expectation_value_width<2>(IRealWavefunction<2>*,
                                                          ...
                                                          MPI_Comm,
                                                          const std::optional<double>);
template std::array<double, 3> expectation_value_width<3>(IRealWavefunction<3>*,
                                                          const LA::MPI::Vector&,
                                                          const Point<3>&,
                                                          MPI_Comm,
                                                          const std::optional<double>);
