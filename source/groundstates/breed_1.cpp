...
/*************************************************************************************************/
  ...
  void MySolver<dim>::assemble_system ()
  {
    ...
    for (; cell!=endc; ++cell)
    {
      ...
      for( unsigned qp=0; qp<n_q_points; qp++ )
      {
        ...
        Q2 = Potential.value(fe_values.quadrature_point(qp)) - m_mu + 3.0*m_gs*u2;
  ...
  int MySolver<dim>::DoIter ( string path )
  {
    ...
    do
    {
      ...
      m_Psi_ref.add( -m_t[1]/fabs(m_t[1]), newton_update); 
      ...
      if( l2norm_t() < 1e-4 ) retval=Status::ZERO_SOL; 
      if( m_res < m_epsilon[1] ) retval=Status::SUCCESS; 
    }while( retval==Status::CONTINUE );
...
int main ( int argc, char *argv[] )
{
  ...
  BreedSolver::MySolver<1> solver("params.xml");
  //solver.run(); 
  solver.run2b(); 
return EXIT_SUCCESS;
}