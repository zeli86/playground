...
namespace BreedSolver
{
  ...
  double fun_neu( const gsl_vector* x, void *params )
  {
    ...
    sol->set_t( gsl_vector_get(x,0), gsl_vector_get(x,1), gsl_vector_get(x,2) );
  ...
  public:
    ...
    void set_t( const double a, const double b, const double c ) { m_t[0]=a; m_t[1]=b; m_t[2]=c; };
  ...
  protected:
    ...
    double l2norm_t() { return sqrt(m_t[0]*m_t[0]+m_t[1]*m_t[1]+m_t[2]*m_t[2]); };  
 ...
 */
    ...
    dof_handler_2 (triangulation)
  {
    try
    {
      ...
      m_QN2[0] = int(m_ph.Get_Physics("QN2",0));
      m_QN2[1] = int(m_ph.Get_Physics("QN2",1));
      m_QN2[2] = int(m_ph.Get_Physics("QN2",2));
  ...
  void MySolver<dim>::setup_system()
  {
    ...
    m_Psi_3.reinit (locally_owned_dofs, mpi_communicator);
  ...
  int MySolver<dim>::DoIter ( string path )
  {
    ...
    do
    {
      ...
      m_Psi_3.add( -m_t[2]/fabs(m_t[2]), m_newton_update); 
      constraints.distribute(m_Psi_3);
  ...
  void MySolver<dim>::run()
  {
    ...
    compute_E_lin( m_Psi_2, T2, N2, W2 );
    //output_guess();

    m_mu = (T+T2)/(N+N2)+m_gs/fabs(m_gs)*m_dmu;
    ...
    for ( ; cell!=endc; ++cell )
    {
      if( cell->is_locally_owned() )
      {
        ...
        for ( unsigned qp=0; qp<n_q_points; qp++ )
        {
          ...
          Q2 = Potential.value(fe_values.quadrature_point(qp)) - m_mu + 3.0*pq;
