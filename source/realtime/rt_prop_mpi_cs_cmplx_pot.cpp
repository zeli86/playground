...
namespace realtime_propagation
{
  ...
  protected:
    ...
    MyTable m_table;
    FunctionParser<dim> m_cmplx_pot;
  };
    ...
    m_cmplx_pot(1)
  {
    ...
    string cmplx_pot_str = "10*(x/" + to_string(m_xmax) + ")^14+10*(y/" + to_string(m_ymax) + ")^14";    
    m_cmplx_pot.initialize( FunctionParser<dim>::default_variable_names(), cmplx_pot_str, constants );
  ...
  void MySolver<dim>::assemble_system ( LA::MPI::Vector& vec )
  {
    ...
    double JxW, pot, cpot, tmp1a, tmp1b, tmp2, sum_re, sum_req, sum_im, sum_imq;
    ...
    for (; cell!=endc; ++cell)
    {
      if( cell->is_locally_owned() )
      {
        ...
        for( unsigned int qp=0; qp<n_q_points; qp++ )
        {
          ...
          cpot = fak2*m_cmplx_pot.value(fe_values.quadrature_point(qp));
          ...
          for (unsigned int i=0; i<dofs_per_cell; i++ )
          {
            ...
            {
              cell_matrix(i,j) += JxW*(1.0-tmp2-cpot)*fe_values[rt].value(i,qp)*fe_values[rt].value(j,qp);
              cell_matrix(i,j) += JxW*(1.0+tmp2-cpot)*fe_values[it].value(i,qp)*fe_values[it].value(j,qp);
  ...
  void MySolver<dim>::assemble_rhs ( LA::MPI::Vector& vec )
  {
    ...
    double JxW, pot, cpot, tmp1, sum_re, sum_im;
    ...
    for (; cell!=endc; ++cell)
    {
      if( cell->is_locally_owned() )
      {
        ...
        for( unsigned int qp=0; qp<n_q_points; qp++ )
        {
          JxW = fe_values.JxW(qp)*fabs(fe_values.quadrature_point(qp)[1]);
          pot = Potential.value(fe_values.quadrature_point(qp));
          cpot = m_cmplx_pot.value(fe_values.quadrature_point(qp));
          ...
          for (unsigned int i=0; i<dofs_per_cell; i++ )
          {
            cell_rhs(i) -= JxW*fak2*((Psi_grad[qp][1]+Psi_t_grad[qp][1])*fe_values[rt].gradient(i,qp) + (pot*sum_im-cpot*sum_re)*fe_values[rt].value(i,qp));
            cell_rhs(i) += JxW*fak2*((Psi_grad[qp][0]+Psi_t_grad[qp][0])*fe_values[it].gradient(i,qp) + (pot*sum_re-cpot*sum_im)*fe_values[it].value(i,qp));
