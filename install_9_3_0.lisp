...
(defvar *packages*
  '((:name openmpi :version "4.1.1"
    ...
    (:name nlopt :version "2.7.0"
     ...
     :path "nlopt-2.7.0")
    (:name lapack :version "3.9.1"
     :url "https://github.com/Reference-LAPACK/lapack/archive/v3.9.1.tar.gz"
     :path "lapack-3.9.1")
    (:name gsl :version "2.7"
     :url "ftp://ftp.gnu.org/gnu/gsl/gsl-2.7.tar.gz")
    (:name muparser :version "2.2.6.1"
     :url "https://github.com/beltoforion/muparser/archive/v2.3.2.tar.gz"
     :path "muparser-2.3.2")  
    (:name p4est :version "2.3.2"
     :url "https://p4est.github.io/release/p4est-2.3.2.tar.gz")
    (:name petsc :version "3.15.1"
     :url "http://ftp.mcs.anl.gov/pub/petsc/release-snapshots/petsc-lite-3.15.1.tar.gz"
     :path "petsc-3.15.1")
    (:name deal.ii :version "9.3.0"
     :url "https://github.com/dealii/dealii/releases/download/v9.3.0/dealii-9.3.0.tar.gz")
...
(defmethod install ((sw-name (eql 'nlopt)) version)
  ...
  (let* ((name (string-downcase (symbol-name sw-name)))
    ...
    (run (format nil "cmake -DNLOPT_CXX=ON -DNLOPT_GUILE=OFF -DNLOPT_LINK_PYTHON=OFF -DNLOPT_MATLAB=OFF \\
